...
export default function ManagerDashboard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const enrichedRecords: PayrollWithEmployee[] = (data.payrollRecords || []).map((record) => {
  ...
  const handleTimeOffAction = async (requestId: number, action: 'approved' | 'rejected') => {
    ...
    try {
      const res = await fetch(`/api/time-off/${requestId}`, {
        ...
        body: JSON.stringify({ status: action, reviewed_by: 1 }),
      });
          ...
          <div className="bg-white shadow rounded-lg overflow-hidden">
            ...
            {employeesWithStatus.length === 0 ? (
              <div className="px-6 py-8 text-center text-gray-500">
                No employee data found. Connect to the database to view attendance.
              </div>
            ) : (
