...
export default async function AttendanceHistory({ userId }: { userId: string }) {
  const attendances = await prisma.attendance.findMany({
    ...
    include: { class: { select: { name: true, instructor: true } } },
  });
      ...
      {attendances && attendances.length > 0 ? (
        <div className="space-y-3">
          {attendances.map((att: AttendanceItem) => (
            ...
            >
              <div>
                <p className="font-medium text-gray-800">{att.class?.name}</p>
                <p className="text-sm text-gray-500">
                  {new Date(att.attended_at).toLocaleString()}
                </p>
                <p className="text-xs text-gray-400">Instructor: {att.class?.instructor}</p>
              </div>
