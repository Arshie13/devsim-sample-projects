...
export default async function AttendanceHistory({ userId }: { userId: string }) {
  const attendances = await prisma.attendance.findMany({
    ...
    include: { classes: { select: { name: true, instructor: true } } },
      ...
      {attendances.length > 0 ? (
        <div className="space-y-3">
          {attendances.map((att) => (
            ...
            >
              <div>
                <p className="font-medium text-gray-800">{att.classes?.name}</p>
                <p className="text-sm text-gray-500">
                  {new Date(att.attended_at).toLocaleString()}
                </p>
                <p className="text-xs text-gray-400">Instructor: {att.classes?.instructor}</p>
              </div>
